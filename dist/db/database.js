...
import { Pool } from "pg";
const defaultDatabaseUrl = "postgresql://rentcars:rentcars@localhost:5432/rentcars";
...
export async function queryFirst(text, params = [], client) {
    const rows = await queryRows(text, params, client);
    return rows[0] ?? null;
}
export async function execute(text, params = [], client) {
    const executor = client ?? pool;
...
async function syncCarReferenceData(client) {
    ...
    await ensureReferenceColumn(client, "car_categories", "seo_text", "TEXT NOT NULL DEFAULT '<p></p>'");
    ...
    await ensureReferenceColumn(client, "car_brands", "seo_text", "TEXT NOT NULL DEFAULT '<p></p>'");
    await ensureReferenceColumn(client, "car_cities", "subdomain", "TEXT");
    await ensureReferenceColumn(client, "car_cities", "seo_title", "TEXT");
    await ensureReferenceColumn(client, "car_cities", "seo_text", "TEXT NOT NULL DEFAULT '<p></p>'");
...
async function ensureReferenceColumn(client, table, column, definition) {
...
async function seedCategories(client) {
    const rows = await client.query("SELECT id, slug, name, seo_title, seo_text FROM car_categories");
    for (const seed of CATEGORY_SEEDS) {
        const existing = rows.rows.find((row) => row.slug === seed.slug ||
            row.id === seed.slug ||
            row.id === seed.id);
        if (!existing) {
            await client.query(`INSERT INTO car_categories (id, slug, name, seo_title, seo_text, sort_order)
                 VALUES ($1, $2, $3, $4, $5, $6)
                 ON CONFLICT (id) DO UPDATE SET
                    ...
                    sort_order = EXCLUDED.sort_order`, [seed.id, seed.slug, seed.name, seed.seoTitle, seed.seoText, seed.sortOrder]);
            continue;
        }
        if (existing.id !== seed.id) {
            await client.query(`INSERT INTO car_categories (id, slug, name, seo_title, seo_text, sort_order)
                 VALUES ($1, $2, $3, $4, $5, $6)
                 ON CONFLICT (id) DO NOTHING`, [seed.id, seed.slug, seed.name, seed.seoTitle, seed.seoText, seed.sortOrder]);
            await client.query(`UPDATE cars SET category_id = $1 WHERE category_id = $2`, [seed.id, existing.id]);
            await client.query(`DELETE FROM car_categories WHERE id = $1`, [existing.id]);
        }
        await client.query(`UPDATE car_categories
             SET slug = $2, name = $3, seo_title = $4, seo_text = $5, sort_order = $6
             WHERE id = $1`, [seed.id, seed.slug, seed.name, seed.seoTitle, seed.seoText, seed.sortOrder]);
    }
}
async function seedBrands(client) {
    const rows = await client.query("SELECT id, slug, name, image_url, seo_title, seo_text FROM car_brands");
    for (const seed of BRAND_SEEDS) {
        ...
        if (!existing) {
            await client.query(`INSERT INTO car_brands (id, slug, name, image_url, seo_title, seo_text, sort_order)
                 ...
                 ON CONFLICT (id) DO UPDATE SET
                    ...
                    seo_title = EXCLUDED.seo_title,
                    seo_text = EXCLUDED.seo_text,
                    sort_order = EXCLUDED.sort_order`, [seed.id, seed.slug, seed.name, seed.imageUrl ?? null, seed.seoTitle, seed.seoText, seed.sortOrder]);
            continue;
        }
        if (existing.id !== seed.id) {
            await client.query(`INSERT INTO car_brands (id, slug, name, image_url, seo_title, seo_text, sort_order)
                 VALUES ($1, $2, $3, $4, $5, $6, $7)
                 ON CONFLICT (id) DO NOTHING`, [seed.id, seed.slug, seed.name, seed.imageUrl ?? null, seed.seoTitle, seed.seoText, seed.sortOrder]);
            await client.query(`UPDATE cars SET brand_id = $1 WHERE brand_id = $2`, [seed.id, existing.id]);
            await client.query(`DELETE FROM car_brands WHERE id = $1`, [existing.id]);
        }
        await client.query(`UPDATE car_brands
             SET slug = $2, name = $3, image_url = $4, seo_title = $5, seo_text = $6, sort_order = $7
             WHERE id = $1`, [seed.id, seed.slug, seed.name, seed.imageUrl ?? null, seed.seoTitle, seed.seoText, seed.sortOrder]);
    }
}
async function seedCities(client) {
    const rows = await client.query("SELECT id, name, subdomain, seo_title, seo_text, address, phone, email, map FROM car_cities");
    for (const seed of CITY_SEEDS) {
        const existing = rows.rows.find((row) => row.id === seed.id ||
            row.subdomain === seed.subdomain ||
            row.name === seed.name ||
            seed.legacyIds?.includes(row.id));
        if (existing && existing.id !== seed.id) {
            await client.query(`INSERT INTO car_cities (id, name, subdomain, seo_title, seo_text, address, phone, email, map, sort_order)
            ...
            await client.query(`UPDATE cars SET city_id = $1 WHERE city_id = $2`, [seed.id, existing.id]);
...
async function seedColors(client) {
    const rows = await client.query("SELECT id, slug, name, hex FROM car_colors");
    for (const seed of COLOR_SEEDS) {
        const existing = rows.rows.find((row) => row.id === seed.id ||
            row.name === seed.name ||
            seed.legacyIds?.includes(row.id));
        if (existing && existing.id !== seed.id) {
            await client.query(`INSERT INTO car_colors (id, name, slug, hex, sort_order)
                 VALUES ($1, $2, $3, $4, $5)
                 ON CONFLICT (id) DO NOTHING`, [seed.id, seed.name, seed.slug, seed.hex, seed.sortOrder]);
            await client.query(`UPDATE cars SET color_id = $1 WHERE color_id = $2`, [seed.id, existing.id]);
        ...
        await client.query(`INSERT INTO car_colors (id, name, slug, hex, sort_order)
             ...
             ON CONFLICT (id) DO UPDATE SET
                ...
                slug = EXCLUDED.slug,
                hex = EXCLUDED.hex,
                sort_order = EXCLUDED.sort_order`, [seed.id, seed.name, seed.slug, seed.hex, seed.sortOrder]);
    }
}
async function seedBodyTypes(client) {
    const rows = await client.query("SELECT id, slug, name FROM car_body_types");
    for (const seed of BODY_TYPE_SEEDS) {
        ...
        if (existing && existing.id !== seed.id) {
            ...
            await client.query(`INSERT INTO car_body_types (id, slug, name, sort_order)
                 VALUES ($1, $2, $3, $4)
                 ON CONFLICT (id) DO NOTHING`, [seed.id, temporarySlug, seed.name, seed.sortOrder]);
            await client.query(`UPDATE cars SET body_type_id = $1 WHERE body_type_id = $2`, [seed.id, existing.id]);
            await client.query(`DELETE FROM car_body_types WHERE id = $1`, [existing.id]);
            await client.query(`UPDATE car_body_types
                 SET slug = $2, name = $3, sort_order = $4
                 WHERE id = $1`, [seed.id, seed.slug, seed.name, seed.sortOrder]);
            continue;
        }
        await client.query(`INSERT INTO car_body_types (id, slug, name, sort_order)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (id) DO UPDATE SET
                slug = EXCLUDED.slug,
                name = EXCLUDED.name,
                sort_order = EXCLUDED.sort_order`, [seed.id, seed.slug, seed.name, seed.sortOrder]);
    }
}
